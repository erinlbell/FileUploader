...
export class FileUploader {
  ...
  public uploadAll():void {
    let items = this.getItemsNotUploaded().filter((item:any) => !item.isUploading);
    if (!items.length) {
      return;
    }
  ...
  public cancelAll():void {
    let items = this.getItemsNotUploaded();
  ...
  public getItemsNotUploaded():Array<any> {
  ...
  private _getTotalProgress(value:number = 0):number {
    ...
    let notUploaded = this.getItemsNotUploaded().length;
